...
void Client::send_data() {
  ...
  msgs->AddRandNums();
  //整形数组intMSG转为字符串，存储在类中的变量strMSG中
  ...
  if (1) {  // Huffman
    ...
    auto huf = msgs->huf;
    // 发送字典
    socket->write(("H" + huf->getMat() + huf->getSendBuf()).toUtf8().data());
  } else {
    msgs->TransToBinary();
    socket->write(("B" + msgs->strMSG).toUtf8().data());
  }
  ...
  //  qDebug() << "txt = " << txt;
  //  huf->calcDict();  //计算字典
  //  auto hufTxt = huf->getSendBuf(txt);
  //  QString mat = huf->getMat();
  //  qDebug() << "mat = " << mat;
  //  //  socket->write(txt.toUtf8().data());
  //  ui->textEdit_read->append(
  //      QString("[local]: hufTxt sent: size=%1").arg(hufTxt.size()));
  //  qDebug() << "hufTxt size = " << hufTxt.size();
  //  delete huf;
}
